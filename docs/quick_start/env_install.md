...
!!! TIP
	在日常使用中，经常会出现运行``./rflypilot``后，不小心关闭了当前终端的情况，此时飞控程序便会因为ssh连接的断开而终止，在实际飞行过程中这样的现象会造成极大的安全隐患。为了避免该问题，可以使用如下方式：
	1. 启动飞控程序时采用``nohup ./rflypilot &``指令，程序的调试输出会被动态保存到``nohup.out``文件中。
	2. 通过``tail -f nohup.out``查看飞控的调试输出，按``Ctrl + C``退出查看。
	3. 结束程序采用``pkill rflypilot``。
	通过此方法，飞控程序便不会被轻易结束掉。
